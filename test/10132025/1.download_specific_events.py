...
agency = "ISC"
NETWORKS = ["UCR","RSPR","JSN","GCG","CATAC",
            "RSNC","IGQ","SJA","VAO","ECX","VAO","BGS","LDG",
            "NAO","UCC","MRB","BUC","RHSSO","PDG",
            "KRSC","BYKL"]

MAX_RETRIES = 5
WAIT_MINUTES = 5
reverse_order = True  # Process contributors in reverse order
# -------------------------
# LOAD AVAILABILITY
# -------------------------
availability = pd.read_csv(availability_path)
availability = availability[(availability["picks"] == True)]
availability = availability[(availability["agency"] == agency)]
...
for i, row in availability.iterrows():
    ...
    for contributor in contributors:
        if contributor not in NETWORKS:
            continue
