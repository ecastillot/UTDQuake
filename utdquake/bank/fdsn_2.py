...
EventID_modes = {"mode_e1":"event.extra.datasource.value+event.extra.eventid.value",
                 "mode_e2":"event.extra.eventid.value",
                 "mode_3":"event.resource_id.id",
                 "mode_4":"event.resource_id.id.split('eventid=')[-1].split('&')[0]",
                 "mode_5":"event.creation_info.agency_id+event.resource_id.id.split('/')[-1]",
                 }
...
class PicksHelper(FDSNClient):
    def __init__(self,*args, **kwargs):
         super().__init__(*args, **kwargs)
    
    def _picks_availability(self):
        natural_mode = self._picks_in_natural_mode()
        if natural_mode["availability"]:
            return natural_mode
        else:
            eventid_mode = self._picks_in_eventid_mode()
            if eventid_mode["availability"]:
                return eventid_mode
            else:
                return {"name":None,"availability": False,
                        "msg":None,"mode":None}
            
    def _picks_in_natural_mode(self):
        
        info = {"name":"natural","availability": False,"msg":None,"mode":None}
    ...
    def _picks_in_eventid_mode(self):
        
        info = {"name":"eventid","availability": False,"msg":None,"mode":None}
        ...
        if "extra" in event.keys():
            ...
            potential_ev_ids = {
                "3": event.resource_id.id,
                "4": event.resource_id.id.split("eventid=")[-1].split("&")[0],
                "5": event.resource_id.id.split("/")[-1],
                "6": event.creation_info.agency_id + event.resource_id.id.split("/")[-1]
            }
        ...
        for p_mode, p_ev_id in potential_ev_ids.items():
            try:
                ...
                cat = self.get_events(eventid=p_ev_id,limit=1)
