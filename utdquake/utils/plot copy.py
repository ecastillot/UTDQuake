...
def synthetic_wavelet(t, t0, kind="P"):
    """
    Returns a synthetic wavelet centered at t0.
    P: more impulsive (short Gaussian)
    S: longer and less impulsive (wider Gaussian)
    """
    if kind == "P":
        sigma = 1.0   # narrow wavelet
        amp = 1.0
    else:  # "S"
        sigma = 3.5   # wider wavelet
        amp = 0.7

    return amp * np.exp(-0.5 * ((t - t0) / sigma)**2) * np.sin(20 * (t - t0))
...
def plot_window_times(
    ...
    last_s_color="cyan",
    phase_column="phase",
    wavelet=True,                 # <<< NEW PARAMETER
    wavelet_length=6               # window width around arrival time
):
    """
    ...
    if not wavelet:
        ...
        # WAVELET MODE (like your reference image)
        all_y = sorted(df["y"].unique())

        for y_val in all_y:
            row = df[df["y"] == y_val]

            # Build a time axis around all arrivals for this row
            t_min = row["window_time"].min() - wavelet_length
            t_max = row["window_time"].max() + wavelet_length
            t = np.linspace(t_min, t_max, 1500)

            trace = np.zeros_like(t)

            for _, r in row.iterrows():
                trace += synthetic_wavelet(t, r["window_time"], r[phase_column])

            # Normalize and shift vertically
            if np.max(np.abs(trace)) > 0:
                trace /= np.max(np.abs(trace)) * 0.8
            trace += y_val

            ax.plot(t, trace, color="black", lw=0.8)
            ax.hlines(y_val, t_min, t_max, color="gray", linestyle="--", lw=0.4)
...
class EQWindow:
    ...
    def _update_timeline(self):
        ...
        if self.event_spacing == "fixed":
            ...
            ev_times = np.fill(ev_w_start_pad,n_events)
        else:
        ...
        # window_end = self.last_allowed_arrival + self

        path ="/groups/igonin/ecastillo/UTDQuake/utdquake/utils/window.png"
        ...
        # max_window_time = arrivals["window_time"].max()

        print("Nth tt per event:", nth_tt_per_ev[0:5])
        # print("Last Nth tt per event:", last_nth_tt_per_ev[0:5])
        print("Max Nth tt:", max_nth_tt)
        # print("Max Last Nth tt:", max_last_nth_tt)
        print("Window size:", w)
        print(ev_times[-5:])
        # print(arrivals[["event_id","travel_time","window_time"]])
        print("Last allowed arrival time:", last_allowed_arrival)
        # print(self.event_origins[["event_id","window_time"]])
        # print(self.arrivals[["event_id","travel_time","window_time"]])
        # print(arrivals["window_time"].describe())
        # print("Max window time:", max_window_time)
        # print(events[["event_id","window_time"]].head())

        # print(arrivals)
        # if self.event_spacing == "free":

        # largest_tt_per_ev = tt_per_ev.max()
        # if self.length is None:
        #     self.length = largest_tt_per_ev + largest_tt_per_ev / 2

        # w_size = self.length
        # print(w_size)

        # w_ev_size = w_size - (self.first_event_w + self.last_event_w) * w_size
        # w_ev_it = w_size - w_ev_size # event start time
        # w_ev_et = w_ev_it + w_ev_size # event end time

        # if self.event_spacing == "fixed":
        #     # Evenly spaced start times
        #     if n_events == 1:
        #         event_times = np.array([w_ev_it])
        #     else:
        #         event_times = np.linspace(w_ev_it, w_ev_et, n_events)
        
        # elif self.event_spacing == "random":
        #     # Random start times inside event region
        #     event_times = np.random.uniform(w_ev_it, w_ev_et, n_events)
        ...
        # # print(w_ev_it,w_ev_et,w_ev_size)
        # constraint = event_times + tt_per_ev
        # excess = constraint - w_size

        # if np.any(excess > 0):
        #     # shift all events left so the maximum violation becomes zero
        #     shift = excess.max()
        #     event_times = event_times - shift
        #     event_times = np.clip(event_times, w_ev_it, w_ev_et - tt_per_ev)
        #     print(event_times)

        # print(constraint,excess)

        # if self.event_spacing == "fixed":
            # W_min = min_window_fixed(tt_per_ev)
            # print("W min:",W_min)

            # lattest_eq = w_ev_et - largest_tt_per_ev
            # # lattest_eq = w_ev_size - largest_tt_per_ev
            # print(w_ev_it, lattest_eq)
            # eq_times = np.linspace(w_ev_it, lattest_eq, n_events, endpoint=True)
        ...
        # def get_spacing(event_spacing,times):
        #     if event_spacing == "fixed":
        #         return max(times)
        #     elif event_spacing == "free":
        #         return times
        #     elif event_spacing == "random":
            


        # events["spacing"] = events[f"arrival_time_n{min_phase}"].apply(lambda x: max_time if event_spacing=="fixed" else x  )
        ...
        if self.max_length is None:
            ...
            # keep only travel_time and event_id
            merged = merged[["event_id", "travel_time"]].dropna()

            # sort arrivals per event
            merged_sorted = merged.sort_values(["event_id", "travel_time"])


            # CASE A: reference = -1 → use ALL arrivals 
            if reference == -1:
                # get largest tt per event
                largest_tt_per_ev = merged_sorted.groupby("event_id")["travel_time"].max()
                largest_tt = largest_tt_per_ev.max()  # global largest
                self.max_length = largest_tt + largest_tt / 2

            # CASE B: reference = N → use Nth arrival per event
            else:
                # get the Nth travel time per event if exists
                nth_tt_list = (
                    merged_sorted.groupby("event_id")
                    .nth(reference - 1)["travel_time"]      # nth element (0 indexed)
                    .dropna()
                )

                if nth_tt_list.empty:
                    raise ValueError(
                        f"No events have at least {reference} arrivals. "
                        f"Try a smaller length_reference_rank or length_reference_rank=-1."
                    )

                # global length is the *largest* Nth-arrival time across all events
                nth_tt = nth_tt_list.max()
                self.max_length = nth_tt + nth_tt / 2
        # else:
        ...
        if self.event_spacing == "fixed":
            # Create equally spaced times
            window_times = np.linspace(
                self.min_event_starttime,
                self.max_length,
                n_events,endpoint=False
            )
        else:  # event_spacing == "random"
            # Random times 
            # NOTE: sorting preserves event order
            window_times = np.sort(
                                np.random.uniform(
                                    low=self.min_event_starttime,
        ...
        #  Merge arrivals with station coordinates
        if not self.arrivals.empty:
            # Merge stations into arrivals, keeping existing columns clean
            self.arrivals = self.arrivals.merge(
                self.stations[["station", "latitude", 
                                "longitude","elevation"]],
                on="station",
                how="left"
            ...
            # Warn if any arrival has no latitude or longitude
            missing_coords = self.arrivals[
                self.arrivals["latitude"].isna() | self.arrivals["longitude"].isna()
            ]
        ...
        events["window_time"] = window_times
        
        self.eq_timeline = events[["event_id", "window_time"]].copy()
    ...
    def plot_window(self,
        ...
        if self.arrivals.empty or self.eq_timeline.empty:
        ...
        event_ids = timeline["event_id"].unique()
        phase_mask = {"P": ["P"], "S": ["S"], "both": ["P", "S"]}[show_phases]

        colors = plt.cm.tab20(np.linspace(0, 1, len(event_ids)))
        ...
        for e, event_id in enumerate(event_ids, 1):
            ...
            # Event origin time
            event_start = timeline.loc[timeline["event_id"] == event_id, "window_time"].values[0]
            ...
            print(f"Event #{e}: {event_id}. t={event_start} s, y={y_eq:.2f} km")

            # --------------------------------------------------
            # 3. Plot earthquake star
            # --------------------------------------------------
            if show_earthquakes:
                ax.scatter(event_start, y_eq, marker="*", color=color, s=150, zorder=5)
            ...
            for phase in phase_mask:
                ...
                arrivals_phase["x"] = event_start + (
                    arrivals_phase["time"] - arrivals_phase["origin_time"]
                ).dt.total_seconds()

                arrivals_phase = arrivals_phase[arrivals_phase["x"] <= event_start + self.max_length]

                for _, row in arrivals_phase.iterrows():
                    facecolor = color if row["phase"] == "P" else "none"
                    ax.scatter(
                        row["x"], row["y"], marker="o",
                        facecolors=facecolor, edgecolors=color, s=10
                    )
